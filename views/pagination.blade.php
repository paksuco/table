...
<div class="flex items-start mt-4">
    ...
    <ul class="flex justify-end flex-1" role="navigation">
        ...
        @if ($paginator->onFirstPage())
            <li class="text-gray-300 bg-white rounded-l shadow-sm" aria-text-gray-300="true" aria-label="@lang('pagination.previous')">
        ...
        @else
            <li class="bg-white rounded-l shadow-sm">
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="hidden text-white bg-blue-600 shadow-sm md:block" aria-current="page"><span class="block px-4 py-2 page-link">{{ $page }}</span></li>
                    @else
                        <li class="hidden bg-white shadow-sm md:block"><button type="button" class="px-4 py-2 page-link" wire:click="gotoPage({{ $page }})">{{ $page }}</button></li>
                    @endif
                @endforeach
            @endif
        @endforeach
        ...
        @if ($paginator->hasMorePages())
            <li class="bg-white rounded-r shadow-sm ">
        ...
        @else
            <li class="text-gray-300 bg-white rounded-r shadow-sm" aria-text-gray-300="true" aria-label="@lang('pagination.next')">
